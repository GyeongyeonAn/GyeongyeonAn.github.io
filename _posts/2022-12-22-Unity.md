...
### 2) 게임 오브젝트 검색
- 검색은 항상 많은 비용이 예상 되므로 사용 할 때는 항상 주의를 기울여야 한다. 
- Hierarchy에 올려진 게임오브젝트 개수가 많을 때에는 매 프레임 마다 검색하는 것이 효율적이지 않으므로 검색하고자 하는 오브젝트들을 처음 한번 검색하여 저장 후 시작하는 것을 추천한다.
- 다양한 검색 방법
    - 게임오브젝트를 이름으로 검색하고자 할 때는GameObject의 멤버함수인 Find를 주로 사용한다. 
    - Tag로 검색 할 시에는 FindWithTag메서드를 사용한다. 
    - 복수개의 게임오브젝트는 FindGameObjectsWithTag 메서드로 동일한Tag의 게임오브젝트를 검색 한다.
    - 타입으로 검색하고자 할 때는 FindObjectOfType 메서드를 사용할 수 있다. 
        - **타입으로 검색 하였을 경우 스크립트의 함수 호출시 GetComonent를 호출하지 않는 장점이 있다.** 
...
### 4) Log 게임 리소스와 프리펩
- 게임리소스는 게임에서 사용하는 자원을 일컫는 말이다. 
- 게임리소스를 사용하기 위해서 프리팹(Prefab)이라는 게임오브젝트를 사용해야 한다. (반드시 프리팹 이어야만 리소스로 사용가능 한 것은 아니다.)
...
public class ResourceLoad : MonoBehaviour {
    ...
    void Start()
    {
        ...
        GameObject tmp_1 = (GameObject)Resources.Load("Cargo");
...
### 6) 이벤트 함수
- Awake() : MnoBehaviour를 상속 받은 스크립트 컴포넌트의 활성/비활성에 상관 없이 실행 되며, Start는 비활성화
- OnEnable() : 게임 시작 이후, 첫 프레임 시작 전, 게임오브젝트가 활성화 될 때 호출되는 함수
- Start() : 게임 시작 이후, 첫 프레임 시작 전, Update 직전에 단 한번 호출되는 함수
- FixedUpdate() : 게임 오브젝트가 Enable 되어 있는 동안, 설정된 고정 시간 주기로 실행되는 함수. 물리 컴포넌트 Update에 활용한다
- Update() : 게임 오브젝트가 Enable 되어 있는 동안 매 프레임마다 반복 호출되는 함수
- LateUpdate() : Update함수중 가장 마지막에 호출되는 Update함수
- OnDisable() : 
    - 게임 오브젝트가 비활성화 될 때(꺼질때) 마다 호출되는 함수. 
    - 호출 시에는 Update 가 대기 상태가 되며 정지된다. 
    - 다시 활성화 할 경우, OnEnable 부터 실행 된다.
- OnApplicationQuit() : Application 종료 시 호출 되는 함수.
...
### 7) 시간 제어
- Time.deltaTime
    - 마지막 프레임을 완료하는 데 걸린 시간 (초) 이다.
    - 이 기능을 사용하면 게임 프레임 속도를 독립적으로 설정할 수 있다.
    - 다른 표현으로 설명하자면 Update 함수 호출 후 다시 자신의 Update 함수가 호출 되기까지의 시간이라고 말할 수 있다.
- Time.time 
    - 게임이 시작된 이후의 시간을 초 단위로 나타낸 것이다. 
    - Start함수에서  Debug.Log(Time.time) 사용하면  0이 출력되고 Update함수에서는 값이 계속 증가할 것이다.
- Time.timeScale 
    - 시간이 지나가는 스케일을 의미하며 슬로우 모션 효과에 사용할 수 있다.
...
``` c#
public class DerectionVector3_2 : MonoBehaviour {
    Vector3 vStart;
...
``` c#
public class Vector3Example_1 : MonoBehaviour {
    void Start()
    {
        Vector3 tmpPos = new Vector3(10f, 10f, 10f);
        Debug.Log("tmpPos의 값 = " + tmpPos.x + tmpPos.y + tmpPos.z);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 이름 = " + gameObject.name);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 전역좌표 = " + transform.position);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 지역좌표 = " + transform.localPosition);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 전역회전값(도단위) = " + transform.eulerAngles);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 지역회전값(도단위) = " + transform.localEulerAngles);
        Debug.Log("이 컴폰넌트가 추가된 게임오브젝트의 크기 = " + transform.localScale);
    }
}
```